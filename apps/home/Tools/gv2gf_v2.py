...
def get_video_info(exiftool_executable, input_path):
    ...
    data_list = []
    print("Keys: ",available_keys)
    print('intersection', set(available_keys).intersection(set(required_values)))
...
def split_video_img(ffmpeg_executable, input_path, output_path, start_secs):
    ...
    # res = run_command(ffmpeg_executable, '-ss', str(start_secs), '-i', input_path, '-frames:v', '1', '-s', '640x640', output_path)
...
def set_csv_row(output_path, geo_data):
    item = {}
    path_list = output_path.split("\\") # can't use r"\\" or r"/"
...
def geovideo_to_geoframes(exiftool_executable, ffmpeg_executable, video_info, df_frames, input_path, output_directory, frame_rates, time_mode):
    ...
    while True:
        ...
        exif_res, geo_data = update_splited_video_geo(exiftool_executable, output_path, start_time, start_secs,
    ...
    file_name = path_list[-2] + ".csv"
    csv_file_name = "\\".join(path_list[0:len(path_list)-1]) + "\\" + file_name
...
def main_process(args):
    ...
    geovideo_to_geoframes(exiftool_executable, ffmpeg_executable, video_info, df_frames,
