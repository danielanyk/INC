...
model = dict(
    backbone=dict(
        ...
        # norm_cfg=dict(  
        #     type='GN',  # Type of norm layer, usually it is BN or GN
        #     requires_grad=True),  # Wh
    ...
    roi_head=dict(
        ...
        # test_cfg=dict(
        # rpn=dict(nms=dict(type='nms', iou_threshold=0.1)),
        # rcnn=dict(
        #     score_thr=0.05,
...
param_scheduler = [
    ...
    # dict(type='ReduceOnPlateauLR', monitor = 'coco/bbox_mAP_50' , rule   = 'greater' , verbose=True   , patience = 6 , factor =0.4 , by_epoch = True )
]
train_cfg = dict(max_epochs=75)



######################### data ################################

data_root ='/home/p2112589/mmdetection/fifnalexternal-2'
