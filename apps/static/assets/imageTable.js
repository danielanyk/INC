...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.handleConfirmClick = function () {
    ...
    console.log("Confirmed - starting processing");

    // === Your original logic ===
    let insp_date = document.getElementById('insp_date').value;
    if (insp_date === '') {
      const today = new Date();
      const day = today.getDate();
    ...
    let tog1 = checktoggle("toggle1");
    let tog2 = checktoggle("toggle2");
    let tog3 = checktoggle("toggle3");
  ...
  stopButton.addEventListener('click', () => {
    ...
    fetch('/stop_process', {
      ...
      body: JSON.stringify({})  // No need to send user_id if not needed
    })
